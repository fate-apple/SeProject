...
class NerTrainer(Trainer):
    ...
    def _train_epoch(self,data):
        ...
        for step,batch in enumerate(data):
            ...
            input_ids,token_type_ids,attention_mask,y,l_is_begin = \
                input_ids.to(self.device),token_type_ids.to(self.device),attention_mask.to(self.device),y.to(self.device),l_is_begin.to(self.device)
            ...
            self.is_head.append(l_is_begin.cpu().detach())
            #if (step+1) % self.empty_cache==0:
                #torch.cuda.empty_cache()
        print(f"\n{'-'*25} train result : {'-'*25}")
        self.outputs = torch.cat(self.outputs,dim=0).cpu().detach()
        self.targets = torch.cat(self.targets,dim=0).cpu().detach()
